...
public class EventResultSetHandler extends AbstractResultSetHandler {
    public EventResultSetHandler(String  index, IndexBuildQueueManager indexBuildQueueManager) {
    ...
    public void handle(ResultSet resultSet) throws Exception {
        ...
        while (resultSet.next()){
            ...
            for (int i = 1; i <= columnCount; i++) {
                ...
                if("industryIds".equals(columnName)&& StringUtils.isEmpty(columnValue)){
                    List<String> industryIds = new ArrayList<String>();
                    String[] ids = columnValue.toString().split(",");
                    for (String id : ids) {
                        industryIds.add(id);
                    }
                    map.put(columnName,industryIds);
                }else if("investSideJson".equals(columnName)&&!StringUtils.isEmpty(columnValue)){
                    map.put(columnName, JSONObject.parseObject(columnValue.toString()).get("investSideJson"));
                }else{
                    map.put(columnName,StringUtils.isEmpty(columnValue)?null:columnValue);
                }
            }
