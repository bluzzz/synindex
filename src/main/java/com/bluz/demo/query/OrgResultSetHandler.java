...
public class OrgResultSetHandler extends AbstractResultSetHandler{
    public OrgResultSetHandler(String index, IndexBuildQueueManager indexBuildQueueManager) {
    ...
    public void handle(ResultSet resultSet) throws Exception {
        ...
        while (resultSet.next()){
            ...
            for (int i = 1; i <= columnCount; i++) {
                ...
                }else if("investStage".equals(columnName)&&!StringUtils.isEmpty(columnValue)){
                    List<String> list = new ArrayList<String>();
                    String[] rounds = columnValue.toString().split(",");
                    for (String round : rounds) {
                        list.add(round);
                    }
                    map.put("investRounds",list);
                }else{
